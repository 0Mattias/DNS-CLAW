...
static int encode_qname(const char *name, uint8_t *buf, size_t buflen)
{
    ...
    while (*p) {
        ...
        if (label_len > 63 || pos + 1 + label_len >= buflen)
            return -1;
 ...
 */
static int decode_name(const uint8_t *msg, size_t msglen, size_t off, char *out, size_t out_len)
{
    size_t pos = off;
    size_t out_pos = 0;
    int consumed = -1;
...
int dns_build_query(uint16_t id, const char *qname, uint8_t *buf, size_t buflen)
{
    ...
    put16(buf + pos, DNS_TYPE_TXT);
    pos += 2;
    put16(buf + pos, DNS_CLASS_IN);
    pos += 2;
    ...
    *rcode = msg[3] & 0x0F;
    ...
    for (uint16_t i = 0; i < ancount; i++) {
        ...
        if (rtype == DNS_TYPE_TXT && txt_pos == 0) {
            ...
            if (rdata_end > msglen)
                return -1; /* malformed: RDATA exceeds packet */
            while (pos < rdata_end) {
                uint8_t slen = msg[pos++];
                if (pos + slen > msglen || pos + slen > rdata_end)
                    return -1;
                if (txt_pos + slen >= txt_out_len)
                    return -1;
...
{
    struct in_addr ia;
    if (inet_pton(AF_INET, server_ip, &ia) != 1)
        return -1;
                  ...
                  size_t txt_out_len)
{
    ...
    int rcode;
    return dns_parse_txt_response(rbuf, (size_t)rlen, &rcode, txt_out, txt_out_len);
}
                  ...
                  char *txt_out, size_t txt_out_len)
{
    ...
    if (SSL_write(ssl, lenbuf, 2) != 2 || SSL_write(ssl, qbuf, qlen) != qlen) {
        SSL_shutdown(ssl);
        SSL_free(ssl);
        SSL_CTX_free(ctx);
        close(fd);
...
static size_t doh_write_cb(void *contents, size_t size, size_t nmemb, void *userp)
{
                  ...
                  size_t txt_out_len)
{
    ...
    int rcode;
    return dns_parse_txt_response(resp_data, resp.len, &rcode, txt_out, txt_out_len);
}
