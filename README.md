...
- **Persistent sessions** — Conversations save to disk automatically; resume with `/sessions` and `/resume <id>`
- **Client auth** — `AUTH_TOKEN` prevents unauthorized access to your server (auto-generated by setup.sh)
...
- **Web UI** — Next.js frontend that tunnels through DNS — same bypass, no terminal needed
...
Point it at your running DNS-CLAW server from the sidebar (address, port, transport, PSK, auth token). Every message goes through DNS queries — the web UI is a TypeScript reimplementation of the C client protocol (DNS wire format, Base32 chunking, AES-256-GCM), not a direct API wrapper.

With Docker:

```bash
docker compose up    # starts both server (port 53) and web UI (port 3000)
```
...
| `/sessions` | List saved sessions on the server |
...
# Client authentication (both client and server must match)
...
When `AUTH_TOKEN` is set, the server rejects any client that doesn't present the matching token. `setup.sh` generates one automatically.

```bash
# Generate manually
openssl rand -hex 16

# Add to ~/.config/dnsclaw/.env (same token on client AND server)
AUTH_TOKEN="your-token-here"
```

Without `AUTH_TOKEN`, the server accepts all connections (backwards compatible).

## Session Persistence

Conversations are automatically saved to `~/.config/dnsclaw/sessions/` on the server after each LLM response. Resume a previous conversation from the client:

```bash
# In the REPL:
/sessions           # list saved sessions with dates
/resume a3f7b2e9    # resume by session ID (prefix match)
```

Disable with `SESSION_PERSIST=false` in your `.env` to keep sessions in-memory only.
...
Or use docker-compose (set env vars in `.env` or export them):

```bash
docker compose up
```

The compose file mounts a named volume for persistent sessions. For DoT/DoH, uncomment the certificate volume mounts in `docker-compose.yml`.
...
├── web/                 # Next.js web frontend (tunnels through DNS)
